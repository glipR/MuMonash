...
int main() {
    ...
    constructST(values, s);

    updateRange(s, 0, s-1, 0);
    ...
    for (int i=0; i<m; i++) {
        ...
        int planks = getSum(s, a-1, b-1);
        if (planks == 0) continue;
        double score = planks / (double) c + planks /(double) (num_planks);
        powers.push(pair<double, int >(score, i));
    }

    ll cost = 0;
    while (getSum(s, 0, s-1) > 0 && powers.size()) {
        pair<double, int> power = powers.top(); powers.pop();
        // cerr << "Selecting " << power.first << " " << power.second << endl;
        int planks = getSum(s, stored_powers[power.second].second.first-1, stored_powers[power.second].second.second-1);
        if (planks == 0) continue;
        // cerr << stored_powers[power.second].second.first-1 << ' ' << stored_powers[power.second].second.second-1 << endl;
        updateRange(s, stored_powers[power.second].second.first-1, stored_powers[power.second].second.first-1 + planks-1, -1);
